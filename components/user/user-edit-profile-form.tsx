"use client";
import { FC, useState, useRef, useEffect, ElementType } from "react";
...
export const UserEditProfileForm: FC<{
  ...
  const submitHandler = async () => {
    ...
    try {
      ...
      const data = await editProfileDetails(
        user.id,
        formDetails,
        uploadedDetails
      );
  ...
  console.log("user.profilePicturePublicId:", user?.profilePicturePublicId);
  console.log({ user });
  return (
    <div className="w-full  flex flex-col gap-4 pt-8  ">
      ...
      <div className="relative flex w-full aspect-[1/0.3] min-h-[150px]">
        <button
          ...
          className={`w-full h-full flex items-center bg-center text-sm ${
            (coverUrl && coverId) ||
            (user?.coverPhotoUrl && user?.coverPhotoPublicId)
              ? ""
        ...
        <button
          ...
          className={`absolute w-[120px] left-[50%] ${
            (profileUrl && profileId) ||
            (user?.profilePictureUrl && user?.profilePicturePublicId)
              ? ""
              : "border border-dashed"
          } flex items-center justify-center translate-x-[-50%] aspect-square rounded-full complete_profile_picture bg-background`}
          onClick={() => fileSelector(profilePicRef)}
      ...
      <div className="w-full  flex flex-col gap-2">
        ...
        <Input
          id="bio"
          spellCheck={false}
          value={websiteName || user.websiteName || ""}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
